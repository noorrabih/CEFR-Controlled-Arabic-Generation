...
def main(args):
    ...
    # Extract CEFR from Document_ID if possible
    if "CEFR" not in essay_df.columns:
        essay_df["CEFR"] = essay_df["Document_ID"].astype(str).str.split("_").str[1]

    # Fallback mapping using levels_csv
    if args.levels_csv and (essay_df["CEFR"].isna().all() or essay_df["CEFR"].nunique(dropna=True) < 2):
        ...
        # if the format of Document_ID is like "essay-12345-B1-xyz", we may need to extract the numeric ID part to match with levels_csv
        if essay_df["Document_ID"].str.contains("-").any():
            essay_df["essay_id"] = essay_df["Document_ID"].astype(str).str.split("-").str[2]
            mapping = dict(zip(levels[args.levels_id_col].astype(str), levels[args.levels_cefr_col].astype(str)))
            essay_df["CEFR"] = essay_df["essay_id"].map(mapping)
        else:
            mapping = dict(zip(levels[args.levels_id_col].astype(str), levels[args.levels_cefr_col].astype(str)))
...
if __name__ == "__main__":
    ...
    ap.add_argument("--levels_id_col", default="ID")
